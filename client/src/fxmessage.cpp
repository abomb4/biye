...
bool FxMessage::toCharStr(char *buffer, int length) const {
    if (length < this->needBufferSize())
        return false;

    // convert from host byte order to network byte order
    uint16_t body_length_l = qToBigEndian(this->_bodylength); // convert 1 total 3
    uint32_t from_user_l = qToBigEndian(this->_from_user); // convert 2 total 3
    uint16_t fno_l = qToBigEndian(this->_fno); // convert 3 total 3
    ...
    while (cur != nullptr) {
        _append(buffer_c, cur->name, cur->l_name);
        _append(buffer_c, ":", 1);
        _append(buffer_c, cur->val, cur->l_val);
        _append(buffer_c, "\n", 1);
        cur = cur->_next;
    }
    return true;
